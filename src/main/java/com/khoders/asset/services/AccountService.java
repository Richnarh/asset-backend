...
public class AccountService {
        ...
        if (billList != null && !billList.isEmpty()){
           try {
               for (Bill bill:billList){
            	   ...
            	   List<BillItem> billItemList = jdbc.query(Sql.BILL_ITEM_BY_BILLID, param, BeanPropertyRowMapper.newInstance(BillItem.class));
    ...
    public BillDto findById(String billId){
        ...
        if (bill != null){
        	SqlParameterSource param = new MapSqlParameterSource(BillItem._billId, bill.getId());
     	   	billItemList = jdbc.query(Sql.BILL_ITEM_BY_BILLID, param, BeanPropertyRowMapper.newInstance(BillItem.class));
            bill.setBillItemList(billItemList);
            return extractMapper.toDto(bill);
        }
        return null;
    ...
    public List<PaymentDto> find(String billOrInvoice, String type){
        ...
        if ((PaymentType.BILL_PAYMENT == PaymentType.valueOf(type))){
        	...
        	paymentList = jdbc.query(Sql.PAYMENT_BY_BILLID, param, BeanPropertyRowMapper.newInstance(Payment.class));
        }else{
        	param = new MapSqlParameterSource(Payment._invoiceId, billOrInvoice);
        	paymentList = jdbc.query(Sql.PAYMENT_BY_INVOICEID, param, BeanPropertyRowMapper.newInstance(Payment.class));
        }
